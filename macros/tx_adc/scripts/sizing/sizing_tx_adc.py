...
# Description: Analytical (gm/ID) sizing of the tx_adc macro.
#
# The specifications live in specs_tx_adc.py — edit that file, not this
...
# scripts/sizing/sizing_tx_adc.md (specs + computed sizing).
#
...
import specs_tx_adc as specs

SPECS_FILE = Path(__file__).resolve().parent / "specs_tx_adc.py"
REPORT_FILE = Path(__file__).resolve().parent / "sizing_tx_adc.md"
# ============================================


def main():
    r = Report("tx_adc", generator=Path(__file__).name)
    r.specs(SPECS_FILE)
    ...
    # Sizing
    # ============================================
    # TODO(tx_adc): size the devices with gm/ID lookups, e.g.:
    #
    ...
    r.text("TODO(tx_adc): implement the sizing calculations.")
