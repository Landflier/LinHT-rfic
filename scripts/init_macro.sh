...
echo "Next steps:"
if [ "$TYPE" = "analog" ]; then
    echo "  1. Fill in the specifications in macros/$NAME/scripts/sizing/specs_$CELL.py"
    echo "     and the gm/ID equations in     macros/$NAME/scripts/sizing/sizing_$CELL.py,"
    echo "     then run 'make sizing' there (writes the report sizing_$CELL.md)"
